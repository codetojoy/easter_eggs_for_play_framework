...
import devtools.DevToolsService;
...
public class PostController extends Controller {
    ...
    public CompletionStage<Result> display(Http.Request request) {
        return handler.find(request).thenApplyAsync(posts -> {
            List<PostResource> postList = posts.collect(Collectors.toList());
            postList.addAll(devToolsService.getTestData());
            return ok(views.html.display.render(postList));
        }, ec.current());
    }
